...
function Adventure(props: SystemCrawlViewProps & { reducedMotion: boolean; effects: EffectState }) {
  ...
  return <div className="sc-terminal sc-adventure">
    <header className="sc-incident-bar">
      ...
      {status !== "connected" && <div className="sc-network-notice" role="status">{status === "reconnecting" ? "Reconnecting — the latest board remains available while controls are paused." : status === "inactive" ? "Inactive — reconnect above to resume with the latest board." : "Offline — the latest board remains available while controls are paused."}</div>}
    </header>
